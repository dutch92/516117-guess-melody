...
describe(`Situation when the player answered less then ${config.QUESTIONS_COUNT} questions`, () => {
...
describe(`Situation when the player answered all questions`, () => {
  it(`Should return ${10 * config.MIN_POINT}, conditions: answered all the questions correctly; not quickly; without fails`, () => {
    const answers = [
    ...
    const attemptsLeft = 4;
    assert.equal(countScore(answers, attemptsLeft), 10 * config.MIN_POINT);
  });

  it(`Should return ${7 * config.MIN_POINT - 3 * config.PENAL_POINT}, conditions: 7 correctly; 3 fails; all aren't quickly`, () => {
    const answers = [
      {isCorrect: true, time: 31},
      {isCorrect: true, time: 32},
    ...
    assert.equal(countScore(answers, attemptsLeft), 7 * config.MIN_POINT - 3 * config.PENAL_POINT);
  });
  ...
  it(`Should return ${config.MAX_SCORE}, the user answered all questions quickly without fails`, () => {
    const answers = [
      {isCorrect: true, time: 3},
      {isCorrect: true, time: 5},
      {isCorrect: true, time: 5},
    ...
    const attemptsLeft = 1;
    assert.equal(countScore(answers, attemptsLeft), config.MAX_SCORE);
  });

  it(`Should return ${3 * config.MAX_POINT + 5 * config.MIN_POINT - 2 * config.PENAL_POINT}, 3 quickly; 5 not quickly; 2 fails`, () => {
    const answers = [
      {isCorrect: true, time: 3},
      {isCorrect: false, time: 5},
    ...
    const attemptsLeft = 1;
    assert.equal(countScore(answers, attemptsLeft), 3 * config.MAX_POINT + 5 * config.MIN_POINT - 2 * config.PENAL_POINT);
  });
});
